...
# This check prevents the OpenTelemetry/WandB tracer from initializing in the
# main process when uvicorn's reloader is active. The tracer initialization
# was modifying source files, causing an infinite reload loop.
# The 'WERKZEUG_RUN_MAIN' env var is set by the reloader in the actual
# worker process.
...
# Initialize dataset configurations and database info before the agent starts
    _dataset_config = load_dataset_config()_database_settings = init_database_settings(_dataset_config)
