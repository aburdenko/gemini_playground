...
This template follows a "bring your own agent" approach - you focus on your business logic in `rag_agent/agent.py`, and the template handles the surrounding components (UI, infrastructure, deployment, monitoring).
...
3.  **Interact and Iterate:**
    ...
    *   Modify the agent logic in `rag_agent/agent.py`. The backend server (FastAPI with `uvicorn --reload`) should automatically restart when you save changes. Refresh the frontend if needed to see behavioral changes.
