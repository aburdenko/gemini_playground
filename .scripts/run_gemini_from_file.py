    ...
    try:
        ...
        if rag_engine_endpoint:
            ...
            try:
                ...
                rag_resource_string = rag_engine_endpoint

                # *** FIX: Re-introduce aggressive cleaning to remove hidden characters (e.g., \r) ***
                # that might be present when reading from the file. This is a common issue with
                # text copied from different operating systems.
                cleaned_rag_resource_string = re.sub(r'[^\w\/-]', '', rag_resource_string.strip())
                
                # The RAG service now supports passing the corpus resource name directly
                if cleaned_rag_resource_string.startswith("projects/") and "/ragCorpora/" in cleaned_rag_resource_string:
                    logger.info(f"    Creating RAG retrieval tool for corpus: '{cleaned_rag_resource_string}'")
                    retrieval = Tool.from_retrieval(
                        grounding.Retrieval(source=grounding.RagSource(rag_corpora=[cleaned_rag_resource_string]))
                    )
                    rag_tool = retrieval
                else:
                    logger.warning(f"    The string from the prompt file '{rag_resource_string}' (cleaned: '{cleaned_rag_resource_string}') does not appear to be a valid RAG Corpus resource name. Skipping RAG.")
            except Exception as e:
